...
Ball& Ball::Fix( int kt, int lt, int lnew )    // перестройка старой траектории
{
    ...
    if( !kt )
        dR=(Vector*)Allocate( lnew*sizeof(Vector),dR );
    return *this;
  ...
  Real mv )               // поляризация нейтрона снимается спиновым гироскопом
{
    if( mPoints>0 )         // происходит управление памятью и списками корпускул
    {
        ...
        if( !mc )
        {
            n=kt=lt=0;    // чистый конструктор
            mc=1000;
            Dip=NULL;
        }
        if( (nc=mPoints)>mc )
            mc=mPoints+120;           // признак расширения списка
        if( mc>m )                                     // увеличение буфера памяти
            Dip=(Ball*)Allocate( mc*sizeof(Ball),Dip );  // и нули остаются на местах
        for( m=0; m<mc; m++ )
            Dip[m].Fix( kt,lt,Fly_count );    // правка траекторий
        lt=Fly_count;      // длина траектории
    }
    else Clear();
    kt=0;
    dT=(MV=mv)/240;    // зачистка поляризованных корпускул
    return *this;                            //
}
Array& Array::Clear()
{ if( mc>0 )
