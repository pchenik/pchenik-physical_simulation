...
{ const Real o=-0.5,i=0.5;
 ...
 #define Vertex( k ) point( out( V[k] ),c[k] )
...
//!    Тор и сфера
static const colors C[]={ gray,lightred,orange,yellow,olive,green,lightgreen,
         cyan,lightcyan,lightblue,purple,lightmagenta,lightgray,white,black };
static const int cp=sizeof( C )/sizeof( colors ); // =15

Model& Model::dipole( _Real s, bool p )          // внутри {-1 <s< +∞ } снаружи
{ const int ns=33,rs=32;                         // ns=33 сопрягается с весовым
  const Vector M={ 1,0,0 };   //{ EqSphere,0,0 } //         коэффициентом 1.253
  int i,j,k; painting_Model _p( p );
  Vector W,Meridian[ns]={ { 0,max( 0.1,s+(s<0?1:exp(-s*s)) ),0 }}; // *EqSphere
         W=dipole_flow( M,Meridian[0] );               // экваториальный отсчет
  for( i=1; i<ns; i++ )                                // четвертинка меридиана
  { Vector V=dipole_flow( M,Meridian[i-1]+W*M_PI/Real( ns ) );
    Meridian[i]=Meridian[i-1] + (( 1.253*V+W )/2.253)*M_PI/Real( ns ); W=V;
  }
  for( i=1-ns; i<ns-1; i++ ){ glBegin( GL_QUAD_STRIP );            // параллели
    for( j=0;  j<=rs;  j++ ){ Real t=(j%rs)*(-2.0*M_PI)/rs;        // меридианы
      for( k=0; k<=1;  k++ ){ W=Meridian[abs(i+k)];                // закраска
        ...
        point( out( (Vector){W.x,W.y*cos(t),W.y*sin(t)} ),C[(i+k+ns)*cp/ns/2] );
//             C[ (ns-i-k)*(cp-1)/ns/2] );
    } } glEnd();
  } return *this;
}
...
Model& Model::sphere( int ns,int rs, bool paint )
{
    ...
    for( int i=0; i<ns; i++ )
    {
        glBegin( GL_QUAD_STRIP );     // rs>=3 - меридианы
        for( int j=0; j<=rs; j++ )
        {
            Real t=(j%rs)*(2.0*M_PI)/rs;
            for( int k=0; k<=1; k++ )
            {
                Real s=M_PI*( 0.5-Real( i+k )/ns );
                point( out( (Vector){sin(s),cos(s)*cos(t),cos(s)*sin(t)} ), // *R ));
...
{ painting_Model _p( paint );
  for( int i=0; i<ns; i++ ){ glBegin( GL_QUAD_STRIP ); // glColor3ubv(C[ns-i-1]);
    for( int j=0; j<=rs; j++ ){ Real t=M_PI*Real(j)*2/rs;
      for( int k=0; k<=1; k++ ){ Real s=M_PI*(Real((i+k)*2+1)/ns+1),d=R+r*cos(s);
      //glNormal3dv( Model.out((Vector){sin(s),cos(s)*cos(t),cos(s)*sin(t)}) );
        point( out( (Vector){ r*sin(-s),d*cos(t),d*sin(t) } ),
